    ...
    try:
        with SQLiteStore(db_path) as store:
            ...
            if is_package_prefix(store, fqn):
                ...
                note = f"> Resolved '{fqn}' → '{resolved}'\n\n" if res.via_suffix else ""
            nodes, edges = QueryEngine(store).get_structural_graph(resolved, max_depth=depth)
    except Exception as exc:
        return f"❌ {exc}"

    return _render_subgraph_with_freshness(
        db_path, output_format, resolved, note, "Structure of", nodes, edges
    )
