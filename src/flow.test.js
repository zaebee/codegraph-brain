...
describe("buildExecutionFlow", () => {
  const makeGraph = (nodes, edges) => ({
    nodes: nodes.map(id => ({ id, name: id, type: "FUNCTION", file_path: "f.py", start_line: 1, end_line: 10 })),
    edges: edges.map(([source, target, type = "CALLS"]) => ({ source, target, type }))
  });
  ...
  it("respects depth limit", () => {
    const graph = makeGraph(["a", "b", "c", "d"], [["a", "b"], ["b", "c"], ["c", "d"]]);
  ...
  it("follows incoming edges", () => {
    const graph = makeGraph(["a", "b", "c"], [["c", "b"], ["b", "a"]]);
  ...
  it("skips non-CALLS edges", () => {
    const graph = {
      nodes: [
        { id: "a", name: "a", type: "FUNCTION", file_path: "", start_line: 1, end_line: 1 },
        { id: "b", name: "b", type: "FUNCTION", file_path: "", start_line: 1, end_line: 1 }
      ],
